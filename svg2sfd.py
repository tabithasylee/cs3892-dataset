...
def print_one_cmd(file, cmd, args):
    scale = 40
    yoff = 720
    result = []
...
def path_to_sfd(file, path):
    ...
    while path.strip() != '':
        ...
        if cmd.upper() == 'M':
            if cmd.islower():
                (x, y), args = apply_rel_xy([x, y], args)
            x0, y0 = args
            print_one_cmd(file, 'm', args)
            x, y = args[-2:]
            if cmd == 'm':
                cmd = 'l'
            elif cmd == 'M':
            ...
            if cmd == 'V':
                ...
                cmd = 'l'
            if cmd.islower():
                args = apply_rel_xy([x, y], args)
            if cmd.upper() == 'S':
                # smooth curveto; reflect
                args = [2 * x - xs, 2 * y - ys] + args
                cmd = 'c'
            print_one_cmd(file, cmd.lower(), args)
            x, y = args[-2:]
            if len(args) > 2:
                xs, ys = args[-4:-2]
        elif cmd.upper() == 'Z':
            if x != x0 or y != y0:
                print_one_cmd(file, 'l', [x0, y0])


def circle_to_sfd(file, cx, cy, r):
    k = 4 * (math.sqrt(2) - 1) / 3
    print_one_cmd(file, 'm', [cx, cy - r])
    print_one_cmd(file, 'c', [cx + k * r, cy - r, cx + r, cy - k * r, cx + r, cy])
    print_one_cmd(file, 'c', [cx + r, cy + k * r, cx + k * r, cy + r, cx, cy + r])
    print_one_cmd(file, 'c', [cx - k * r, cy + r, cx - r, cy + k * r, cx - r, cy])
    print_one_cmd(file, 'c', [cx - r, cy - k * r, cx - k * r, cy - r, cx, cy - r])


def conv_svg(fn, char, glyphnum=None):
    name = os.path.join("sfd", f"{os.path.splitext(os.path.basename(fn))[0]}.sfd")
    with open(name, "w+") as file:
        ...
        file.write(f'''
StartChar: {os.path.basename(fn)[:-4]}
Encoding: {char} {glyphnum} {char}
Width: {(21 * 40)}
Flags: W
LayerCount: 2
Fore
...
''')
        ...
        for path in doc.getElementsByTagName('path'):
            path_to_sfd(file, path.getAttribute('d'))
        for polygon in doc.getElementsByTagName('polygon'):
            path_to_sfd(file, 'M' + polygon.getAttribute('points') + 'z')
        for circle in doc.getElementsByTagName('circle'):
            ...
            circle_to_sfd(file, cx, cy, r)
        file.write ('''EndSplineSet
EndChar''')
        print_footer(file)
